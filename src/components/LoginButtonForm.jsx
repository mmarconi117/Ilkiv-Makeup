...
import { login } from '../api';  // your API login function
...
export default function LoginButtonForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // Assume response.data contains token or other info
      dispatch(loginSuccess({
        username,
        token: response.data.token,  // if available
      }));
  ...
  return (
    <div>
      {loggedIn ? (
        <div>
          <p className="text-white mt-2">Welcome, {storedUsername}!</p>
          <button
